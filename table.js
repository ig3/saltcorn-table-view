...
const { removeEmptyStrings, removeDefaultColor, applyAsync, mergeIntoWhere, mergeConnectedObjects, } = require("@saltcorn/data/utils");
const { field_picker_fields, picked_fields_to_query, stateFieldsToWhere, initial_config_all_fields, stateToQueryString, stateFieldsToQuery, link_view, getActionConfigFields, readState, run_action_column, add_free_variables_to_joinfields, } = require("@saltcorn/data/plugin-helper");
const { get_viewable_fields } = require("@saltcorn/data/base-plugin/viewtemplates/viewable_fields");
const { getState } = require("@saltcorn/data/db/state");
const { get_async_expression_function, jsexprToWhere, freeVariables, } = require("@saltcorn/data/models/expression");
const db = require("@saltcorn/data/db");
const { get_existing_views } = require("@saltcorn/data/models/discovery");
const { InvalidConfiguration, isWeb } = require("@saltcorn/data/utils");
const { check_view_columns } = require("@saltcorn/data/plugin-testing");
const { extractFromColumns, extractViewToCreate, } = require("@saltcorn/data/diagram/node_extract_utils");
/**
...
const configuration_workflow = (req) => new Workflow({
    onDone: async (ctx) => {
        if (ctx.options._create_db_view) {
 ...
 * @returns {Promise<*>}
 */
const run = async (table_id, viewname, { columns, view_to_create, create_view_display, create_view_label, default_state, options, create_view_location, create_link_style, create_link_size }, stateWithId, extraOpts, { listQuery }) => {
    ...
    const rows_per_page = (options && options._rows_per_page) || 20;
    ...
    if (options && options._omit_header) {
        page_opts.noHeader = true;
    }
    page_opts.transpose = (options || {}).transpose;
    page_opts.transpose_width = (options || {}).transpose_width;
    page_opts.transpose_width_units = (options || {}).transpose_width_units;
    ...
    if (view_to_create &&
        ...
        if (create_view_display === "Embedded") {
            ...
            create_link = link_view(hrefVal, __(create_view_label) || `Add ${pluralize(table.name, 1)}`, create_view_display === "Popup", create_link_style, create_link_size);
        }
    }
    const create_link_div = isright
        ? div({ class: "float-end" }, create_link)
        : create_link;
    const tableHtml = mkTable(options?.hide_null_columns
...
module.exports = {
     ...
     * @returns {boolean}
     */
    display_state_form: (opts) => !(opts && opts.options && opts.options._omit_state_form),
    /**
     * @param {object} opts
     * @returns {boolean}
     */
    ...
    queries: ({ table_id, exttable_name, viewname, configuration: { columns, options }, req, }) => ({
        async listQuery(state) {
            const table = await Table.findOne(typeof exttable_name === "string"
                ? { name: exttable_name }
                : { id: table_id });
            const fields = await table.getFields();
            ...
            if (!q.orderBy)
                q.orderBy =
                    (options && options._order_field) || table.pk_name;
            if (!q.orderDesc)
                q.orderDesc = options && options._descending;
            ...
            //console.log({ i: options.include_fml });
            if (options?.include_fml) {
                let where1 = jsexprToWhere(options.include_fml, state);
